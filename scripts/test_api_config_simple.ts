...
async function testApiConnectivity() {
  try {
    ...
    if (
      ...
      try {
        const response = await fetch(
          `https://api.govinfo.gov/collections?api_key=${config.govinfo.apiKey}`,
        );
        if (response.ok) {
          console.log("   ✅ govinfo.gov API key is valid and working");
        } else {
          console.log(
            "   ⚠️  govinfo.gov API key response:",
            response.status,
            response.statusText,
          );
        }
        ...
        console.log(
          "   ⚠️  Unable to test govinfo.gov API connectivity:",
          error.message,
        );
      }
    } else {
    ...
    if (
      ...
      try {
        const response = await fetch(
          `https://api.congress.gov/v3/bill?api_key=${config.congress.apiKey}&limit=1`,
        );
        if (response.ok) {
          console.log("   ✅ congress.gov API key is valid and working");
        } else {
          console.log(
            "   ⚠️  congress.gov API key response:",
            response.status,
            response.statusText,
          );
        }
        ...
        console.log(
          "   ⚠️  Unable to test congress.gov API connectivity:",
          error.message,
        );
      }
    } else {
