  ...
  try {
    ...
    if (config.govinfo && config.govinfo.apiKey && config.govinfo.apiKey !== "YOUR_REAL_GOVINFO_API_KEY_HERE") {
      ...
      try {
        ...
        console.log(`   Testing connectivity to: ${testUrl.substring(0, 50)}...`);
        console.log("   ✅ govinfo API configuration appears valid");
      } catch (error) {
        console.log("   ⚠️  Unable to test govinfo API connectivity (this is normal in this test)");
      }
    } else {
      console.log("   ❌ govinfo API key not configured");
      console.log("   Please edit ingestion/config/api_config.json with your real govinfo API key");
    }
    ...
    if (config.congress && config.congress.apiKey && config.congress.apiKey !== "YOUR_REAL_CONGRESS_API_KEY_HERE") {
      ...
      try {
        ...
        console.log("   ⚠️  Unable to test congress API connectivity (this is normal in this test)");
      }
    } else {
      console.log("   ❌ congress API key not configured");
      console.log("   Please edit ingestion/config/api_config.json with your real congress API key");
    }
    
    // Test Cloudflare configuration
    console.log("\\n3. Testing Cloudflare configuration...");
    if (config.cloudflare && config.cloudflare.accountId && config.cloudflare.accountId !== "YOUR_CLOUDFLARE_ACCOUNT_ID") {
      console.log("   ✅ Cloudflare account ID configured");
    } else {
      console.log("   ⚠️  Cloudflare account ID not configured");
      console.log("   This is optional for local development but required for deployment");
    }
    
    console.log("\\n🎉 Configuration test completed!");
    console.log("\\nNext steps:");
    console.log("1. If any APIs show as not configured, edit ingestion/config/api_config.json");
    console.log("2. Run a test ingestion: npm run ingest:govinfo");
    console.log("3. Check the full configuration guide: autorag/API_CONFIGURATION_GUIDE.md");
