...
export class AnalysisService {
  ...
  static async analyzeSentiment(text: string, env: any): Promise<SentimentAnalysis> {
    ...
    const positiveWords = ['good', 'great', 'excellent', 'positive', 'support', 'benefit', 'improve'];
    const negativeWords = ['bad', 'terrible', 'awful', 'negative', 'oppose', 'harm', 'worsen'];
    ...
    const subjectivity = Math.min(1, (positiveCount + negativeCount) / words.length);
  ...
  static async extractEntities(text: string, env: any): Promise<Entity[]> {
    ...
    const personMatches = text.match(/\b(?:Mr|Mrs|Ms)\.?\s+[A-Z][a-z]+\b/g) || [];
  ...
  static async extractTopics(text: string, env: any): Promise<Topic[]> {
    ...
    const topicKeywords: Record<string, string[]> = {
      "Politics": ["government", "policy", "election", "vote", "congress", "senate"],
      "Economy": ["economy", "financial", "budget", "tax", "employment", "growth"],
      "Healthcare": ["health", "medical", "hospital", "insurance", "patient", "treatment"],
      "Education": ["school", "education", "student", "teacher", "university", "learning"],
      "Environment": ["environment", "climate", "pollution", "energy", "sustainability", "green"]
    };
  ...
  static async extractKeyPhrases(text: string, env: any): Promise<KeyPhrase[]> {
    ...
    const uniquePhrases = [...new Map(phrases.map(item => [item.phrase, item])).values()];
  ...
  static async analyzeDocument(text: string, env: any): Promise<DocumentAnalysis> {
    // Perform all analysis tasks
    const [sentiment, entities, topics, keyPhrases, language] = await Promise.all([
