...
export class PoliticalAnalyzer {
  ...
  static async detectBias(text: string): Promise<BiasAnalysis> {
    ...
    console.log(
      `Detecting political bias in text: ${text.substring(0, 50)}...`,
    );
  ...
  static async analyzeSentiment(text: string): Promise<SentimentAnalysis> {
    ...
    return {
      ...
      confidence: Math.min(
        1,
        (positiveCount + negativeCount + subjectiveCount) / 20,
      ),
      keyPhrases,
    };
  }
  ...
  static async factCheck(text: string): Promise<FactCheckResult> {
    ...
    const factualAccuracy =
      claims.length > 0 ? factualClaims.length / claims.length : 0.8;
