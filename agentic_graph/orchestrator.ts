...
import { AnalysisAgent, PoliticianProfilerAgent, InferenceAgent } from './agents/analysis_agents';
...
export class KnowledgeGraphOrchestrator {
  ...
  async processDocument(document: Document): Promise<void> {
    ...
    try {
      ...
      console.error(`Orchestrator: Error processing document ${document.id}:`, error);
      throw error;
    }
  }
  ...
  async processDocuments(documents: Document[]): Promise<void> {
    ...
    console.log(`Orchestrator: Completed processing ${documents.length} documents`);
  }
    ...
    try {
      ...
      for (const politicianId of politicianIds) {
        try {
          ...
          console.warn(`Orchestrator: Failed to update profile for politician ${politicianId}:`, error);
        }
      }
  ...
  private async storeDocumentMetadata(document: Document, analysisResult: any): Promise<void> {
    ...
    try {
      ...
      console.error(`Orchestrator: Error storing metadata for document ${document.id}:`, error);
      throw error;
    }
  }
  ...
  async getPoliticianInsights(politicianId: string): Promise<any> {
    console.log(`Orchestrator: Getting insights for politician ${politicianId}`);
    ...
    try {
      ...
      console.error(`Orchestrator: Error getting insights for politician ${politicianId}:`, error);
      throw error;
    }
  }

  // Get relationship insights between politicians
  async getRelationshipInsights(politicianId1: string, politicianId2: string): Promise<any> {
    console.log(`Orchestrator: Getting relationship insights between ${politicianId1} and ${politicianId2}`);
    ...
    try {
      const result = await this.neo4j.runQuery(query, { politicianId1, politicianId2 });
      return result.records;
    } catch (error: any) {
      console.error(`Orchestrator: Error getting relationship insights:`, error);
      throw error;
    }
  }
}