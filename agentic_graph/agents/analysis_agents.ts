...
export class AnalysisAgent {
  ...
  async analyzeDocument(documentId: string, content: string): Promise<AnalysisResult> {
    ...
    try {
      ...
      const entityTask = this.createTask('entity_extraction', { documentId, content });
      
      // Create task for political analysis
      const politicalTask = this.createTask('political_analysis', { documentId, content });
      ...
      console.log(`AnalysisAgent: Completed analysis of document ${documentId}`);
      return result;
    } catch (error: any) {
      console.error(`AnalysisAgent: Error analyzing document ${documentId}:`, error);
      throw error;
    }
  }
  ...
  private async processEntityExtraction(task: AgentTask): Promise<any> {
    ...
    try {
      ...
      console.error(`AnalysisAgent: Error in entity extraction task ${task.id}:`, error);
      throw error;
    }
  }
  ...
  private async processPoliticalAnalysis(task: AgentTask): Promise<any> {
    ...
    try {
      ...
      task.status = 'failed';
      task.error = error.message;
      console.error(`AnalysisAgent: Error in political analysis task ${task.id}:`, error);
      throw error;
    }
  }

  // Store results in graph database
  private async storeResults(documentId: string, result: AnalysisResult): Promise<void> {
    ...
    try {
      ...
      for (const entity of result.entities) {
        ...
        if (GraphSchema.validateNode(nodeLabel, { id: entity.text, name: entity.text })) {
      ...
      for (const relationship of result.relationships) {
        ...
        if (GraphSchema.validateRelationship(relationship.predicate, {})) {
          await this.neo4j.createRelationship(
            ...
            relationship.object.label === 'LEGISLATION' ? 'Legislation' : 'GovernmentBody',
      ...
      console.log(`AnalysisAgent: Successfully stored results for document ${documentId}`);
    } catch (error: any) {
      console.error(`AnalysisAgent: Error storing results for document ${documentId}:`, error);
      throw error;
    }
  }
...
export class PoliticianProfilerAgent {
  ...
  async createProfile(politicianId: string): Promise<any> {
    console.log(`PoliticianProfilerAgent: Creating profile for politician ${politicianId}`);
    
    try {
      // Get all statements by this politician
      const statements = await this.neo4j.findNodes('Statement', { speakerId: politicianId });
      
      // Get all votes by this politician
      const votes = await this.neo4j.findNodes('Vote', { politicianId: politicianId });
      ...
      const statementPatterns = await this.analyzeStatementPatterns(statements.records);
      ...
      const profile = {
        ...
        consistencyScore: this.calculateConsistency(statementPatterns, votingPatterns),
      ...
      console.log(`PoliticianProfilerAgent: Created profile for politician ${politicianId}`);
      return profile;
    } catch (error: any) {
      console.error(`PoliticianProfilerAgent: Error creating profile for politician ${politicianId}:`, error);
      throw error;
    }
  }
  ...
  private calculateConsistency(statementPatterns: any, votingPatterns: any): number {
...
export class InferenceAgent {
  ...
  async inferRelationships(): Promise<void> {
    ...
    try {
      ...
      console.error('InferenceAgent: Error during relationship inference:', error);
      throw error;
    }
  }
}