...
export class DatabaseService {
    ...
    try {
      ...
      if (pkg.metadata) {
        for (const [key, value] of Object.entries(pkg.metadata)) {
          ...
          ).bind(pkg.packageId, key, String(value)).run();
        }
      }
      ...
      if (pkg.identifiers) {
        for (const identifier of pkg.identifiers) {
          ...
          ).bind(pkg.packageId, identifier.type, identifier.value).run();
        }
      }
      ...
      if (pkg.downloads) {
        for (const download of pkg.downloads) {
          ...
          ).bind(pkg.packageId, download.type, download.url, download.size).run();
        }
      }
    ...
    try {
      ...
      if (bill.subjects) {
        for (const subject of bill.subjects) {
          ...
          ).bind(bill.billId, subject.name).run();
        }
      }
  ...
  async storeCongressCommittee(committee: any): Promise<void> {
    try {
      ...
      console.error(`Error storing congress committee ${committee.committeeId}:`, error);
      throw error;
    }
  }
}