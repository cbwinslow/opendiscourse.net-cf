...
export class WebhookReceiver {
  ...
  static async handleWebhook(request: Request, env: any): Promise<Response> {
    try {
      ...
      console.log(
        `Received webhook from ${payload.source}: ${payload.eventType}`,
      );
  ...
  private static async processGovInfoWebhook(
  ...
  private static async processCongressWebhook(
    payload: WebhookPayload,
    env: any,
  ): Promise<void> {
  ...
  private static async storeWebhookEvent(
    ...
    console.log(
      `Storing webhook event for later processing: ${payload.source} - ${payload.eventType}`,
    );
  }
