...
export const config = getConfig({
  ...
  ...(process.env.CHUNK_SIZE && {
    chunking: { chunkSize: parseInt(process.env.CHUNK_SIZE, 10) },
  }),
  ...(process.env.CHUNK_OVERLAP && {
    chunking: { chunkOverlap: parseInt(process.env.CHUNK_OVERLAP, 10) },
  }),
  ...(process.env.EMBEDDING_BATCH_SIZE && {
    embedding: { batchSize: parseInt(process.env.EMBEDDING_BATCH_SIZE, 10) },
  }),
  ...(process.env.QUEUE_CONCURRENCY && {
    queue: { concurrency: parseInt(process.env.QUEUE_CONCURRENCY, 10) },
  }),
  ...(process.env.MAX_FILE_SIZE && {
    files: { maxFileSize: parseInt(process.env.MAX_FILE_SIZE, 10) },
  }),
});
