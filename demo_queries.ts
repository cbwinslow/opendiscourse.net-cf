  ...
  return [
    {
      ...
      content:
        "Consumers shall have the right to obtain confirmation as to whether or not personal data concerning them is being processed...",
      similarity: 0.92,
      source: "Bill Text",
    },
    {
      ...
      content:
        "The committee recognizes the importance of protecting consumer privacy in the digital age...",
      similarity: 0.87,
    ...
    },
    {
      ...
      content:
        "Mr. Chairman, I rise today to speak in support of this important privacy legislation...",
      similarity: 0.81,
...
async function askQuestion(question) {
  ...
  if (question.toLowerCase().includes("privacy")) {
    ...
    return {
      answer:
        "Based on the available documents, this appears to be related to legislative processes in Congress. The documents show various bills, committee reports, and congressional records that discuss legislative activities.",
      confidence: 0.75,
      citations: [
...
async function runQueries() {
  try {
    ...
    members.forEach((member, index) => {
      console.log(
        `${index + 1}. ${member.name} (${member.party}-${member.state})`,
      );
    ...
    console.log(
      `Found ${semanticResults.length} semantically relevant documents:`,
    );
    ...
    console.log(
      `Centrality Score: ${(graphResult.centrality * 100).toFixed(1)}%`,
    );
    console.log("Related Entities:");
    graphResult.relatedEntities.forEach((rel, index) => {
      console.log(
        `  ${index + 1}. ${rel.name} - ${rel.relationship} (${(rel.strength * 100).toFixed(1)}%)`,
      );
    });
    console.log("");
