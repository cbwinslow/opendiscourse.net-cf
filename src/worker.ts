iythe /// <reference types="@cloudflare/workers-types/2023-07-01" />
...
const worker = {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    const router: Record<string, (req: Request) => Promise<Response>> = {
      ...
      '/health': async () => new Response(JSON.stringify({ status: 'ok' }), {
    ...
    // Handle the request
    const handler = router[path] || (() => new Response(JSON.stringify({ error: 'Not Found' }), { status: 404 }));
